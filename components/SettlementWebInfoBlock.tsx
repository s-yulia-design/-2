...
export function SettlementWebInfoBlock({ settlementId, webInfo }: Props) {
  ...
  async function searchWeb() {
    ...
    try {
      ...
      const data = (await res.json()) as SettlementWebInfo & { error?: string };
      if (!res.ok) {
        setMessage({ type: "error", text: data.error || "Не удалось найти данные" });
        return;
      }
  ...
  const busy = searchLoading || saveLoading;
